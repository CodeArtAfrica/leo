...
impl Command for LeoCheck {
    ...
    fn prelude(&self, context: Context) -> Result<Self::Input> {
        ...
        LeoBuild { options: self.build_options.clone() }.execute(context)
    }

    fn apply(self, context: Context, _: Self::Input) -> Result<Self::Output> {
        // Parse the network.
        let network: NetworkName = context.get_network(&self.build_options.network)?.parse()?;
        match network {
...
fn handle_check<N: Network>(command: &LeoCheck, context: Context) -> Result<<LeoCheck as Command>::Output> {
    ...
    // Check that the editions of all the remote programs are the latest.
    for (program_id, (_, edition, _)) in remote {
