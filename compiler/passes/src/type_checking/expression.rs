...
impl ExpressionVisitor for TypeCheckingVisitor<'_> {
    ...
    fn visit_tuple(&mut self, input: &TupleExpression, expected: &Self::AdditionalInput) -> Self::Output {
        ...
        // We can't manually construct a tuple with an external record member.
        for expr in &input.elements {
            if let Expression::Identifier(identifier) = expr {
                if let Some(input_name) = self.external_record_inputs.get(&identifier.name) {
                    self.emit_err(TypeCheckerError::external_record_in_tuple(
                        identifier.name,
                        input_name,
                        identifier.span,
                    ))
